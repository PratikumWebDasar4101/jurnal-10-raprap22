<?php
    class data{
        ...
        public function tambahdata($nd, $nb, $nim, $kelas, $hobi, $genre, $wisata, $tanggal, $email){
            ...
            $sql = "INSERT INTO ta9(namadepan,namabelakang,nim,kelas,hobi,genre,wisata,tanggal,email) VALUES ('$nd','$nb','$nim','$kelas','$list_hobi','$list_genre','$list_wisata','$tanggal','$email')";
        ...
        public function editdata($namadepan, $namabelakang, $nim, $kelas, $hobi, $genre, $wisata, $tanggal, $email){
            ...
            $sql = "UPDATE ta9 SET namadepan ='$namadepan',namabelakang ='$namabelakang',kelas ='$kelas',hobi ='$list_hobi',genre ='$list_genre',wisata ='$list_wisata',tanggal ='$tanggal',email ='$email' WHERE nim ='$nim'";
            if ( mysqli_query($this->conn,$sql)) {
                ?>
                <script>
        ...
        public function delete($nim){
            $sql = mysqli_query($this->conn, "DELETE FROM ta9 WHERE nim = '$nim'");
            if ($sql) {
                ?>
                <script>
        ...
        public function tampil_data($nim){
            $sql = mysqli_query($this->conn, "SELECT * FROM ta9 WHERE nim = '$nim'");
            return $sql;
        }

        public function tampil($cari){
            $sql = mysqli_query($this->conn, "SELECT * FROM ta9 WHERE nim LIKE '%$cari%'");
            return $sql;
        }
    }
    $data = new data();
